...
namespace Trees
{
    public partial class Form1 : Form
    {
        ...
        {
            tree = new Tree(new Entry<int,int> (rand.Next(-10, 10), rand.Next(-10, 10)), 0, size);
        ...
        private void Form1_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == 'f')
            {
                ...
                move_y = 0;
                tree = new Tree(new Entry<int, int>(rand.Next(-10, 10), rand.Next(-10, 10)), 0, size);
                for (int i = 0; i < 100; i++) tree.Add(new Entry<int, int>(rand.Next(-100, 100), rand.Next(-100, 100)));
                tree.recalc();
                move_x = -tree.dx;
                redraw = true;
                Invalidate();
            ...
            if (e.KeyChar == 'e')
            {
                tree.Add(new Entry<int, int>(rand.Next(-100, 100), rand.Next(-100, 100)));
                redraw = true;
                Invalidate();
            }
            if (e.KeyChar == 'c')
